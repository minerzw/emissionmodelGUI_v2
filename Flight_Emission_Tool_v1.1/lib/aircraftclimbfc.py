# -*- coding: utf-8 -*-
"""
Created on Wed Mar 30 15:19:16 2022
...
def climb_fc(aircraftproperties, climbprofile, cruise_alt_max, consumption_prop, W_cruise, thrust_var, F_opt_cruise):
    ## Constants
    g = 9.81
    gamma = 1.4
    R = 287
    ...
    vx = climbprofile[:,3]
    vz = climbprofile[:,5]
    theta = climbprofile[:,6]/180*np.pi
    H = climbprofile[:,1]
    T = climbprofile[:,7]
    rho = climbprofile[:,8]
    cd0 = climbprofile[:,10]
    ...
    ## Calculate Mach number, optimum thrust, maximum thrust, idle thrust
    M = v/(gamma*R*T)**(1/2)
    ...
    for i in reversed(range(np.size(t))):
        ...
        if i > 0:
            W[i-1] = W[i] + fc[i]*g*dt 
    total_fuel_climb = scipy.integrate.simps(fc,t) 
    
    t_TO = 35
    TSFC_TO = TSFC_min*TSFC_FT_penalty
    
    # Determine TO fuel consumption
    total_fuel_TO = F_max_TO*TSFC_TO*t_TO
    
    return total_fuel_climb, total_fuel_TO, fc, W, F